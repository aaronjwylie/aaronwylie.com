...
export interface UsageDigest {
  day: string;
  /** Every recorded view: people, bots and unclassified. */
  totalViews: number;
  humanViews: number;
  humanVisitors: number;
  botViews: number;
  /** Recorded before bot classification existed, so counted as neither. */
  unclassifiedViews: number;
  // The fields below, and every top-N list except topBots, cover people only.
  toolViews: number;
  locatedViews: number;
  ...
  topBots: { name: string; views: number }[];
}
...
export async function sendUsageDigest(d: UsageDigest, log: FastifyBaseLogger): Promise<boolean> {
  ...
  const unclassified = d.unclassifiedViews
    ? ` · ${d.unclassifiedViews} unclassified (recorded before bot detection)`
    : '';
  ...
  const html =
    ...
    `<p style="margin:0;font-size:15px"><strong>${d.humanVisitors}</strong> visitors · ` +
    `<strong>${d.humanViews}</strong> page views · ` +
    `<strong>${d.toolViews}</strong> on tools · ${d.locatedViews} geo-located</p>` +
    `<p style="margin:4px 0 0;color:#666;font-size:13px">People only. ` +
    `<strong>${d.botViews}</strong> bot views left out${unclassified}.</p>` +
    ...
    section('Bots (not counted above)', d.topBots.map((b) => ({ label: b.name, views: b.views }))) +
  ...
  const text =
    ...
    `People: ${d.humanVisitors} visitors, ${d.humanViews} page views, ${d.toolViews} on tools, ${d.locatedViews} geo-located\n` +
    `Bots: ${d.botViews} page views left out${unclassified}\n\n` +
    `Top tools:\n${d.topTools.map((t) => `  ${t.path}: ${t.views}`).join('\n') || '  -'}\n\n` +
    `Top countries:\n${d.topCountries.map((t) => `  ${t.country}: ${t.views}`).join('\n') || '  -'}\n\n` +
    `Top cities:\n${d.topCities.map((t) => `  ${t.label}: ${t.views}`).join('\n') || '  -'}\n\n` +
    `Bots (not counted above):\n${d.topBots.map((b) => `  ${b.name}: ${b.views}`).join('\n') || '  -'}\n`;
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        subject: `Portfolio digest - ${d.day} (${d.humanViews} views, ${d.botViews} bot)`,
        html,
        text,
      }),
    });
